...
int exponentialDelay(double mu) {
    ...
    exponential_distribution<double> distribution(1.0 / mu);
...
void* reader(void* arg) {
    ...
    for (int i = 0; i < kr; i++) {
        ...
        coutMutex.unlock();
        //start measuring waiting time here
        ...
        //end measuring waiting time here
...
int main() {
    ...
    for (long i = 1; i <= nw; ++i) { // Creating 2 writer threads for demonstration
        pthread_t tid;
    ...
    waittimes.open("RWAvgWaitTime.txt");
    if (!waittimes) {
