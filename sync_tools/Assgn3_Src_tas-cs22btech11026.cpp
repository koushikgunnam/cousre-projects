...
int rowInc;
int C;
atomic_flag lock = ATOMIC_FLAG_INIT; // Atomic flag for TAS
...
int main() {
    ...
    ofstream coutputFile("outTAS.txt", ios::trunc); // ios::trunc flag to replace the file if it already exists

    //check if the file is opened successfully
    if (!coutputFile.is_open()) {
        cerr << "Error writing outTAS.txt\n";
        return 1;
    }
