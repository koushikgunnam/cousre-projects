...
vector<bool> waiting; // Array to track thread waiting status
...
int main() {
  ...
  //create threads
    for (int i = 0; i < k; i++) {
        cargs[i] = {&matrix, &cresultMatrix, n, k, i};
    ...
    ofstream coutputFile("outBOUNDEDCAS.txt", ios::trunc); // ios::trunc flag to replace the file if it already exists

    //check if the file is opened successfully
    if (!coutputFile.is_open()) {
        cerr << "Error writing outBOUNDEDCAS.txt\n";
        return 1;
    }
